...
The component uses `UDropdownMenu` with a `UButton` as trigger. Use `useFetch` with a `key` to fetch and cache the chat list:
...
::note{target="_blank" to="https://vercel.com/docs/ai-gateway"}
