...
<template>
  <MotionConfig :transition="{ duration: 0.7, type: 'spring', bounce: 0.5 }">
    <Motion
      layout
      as="div"
      class="bg-default/50 flex w-full max-w-lg flex-col gap-4 overflow-hidden rounded-3xl border p-6"
      :animate="{
        height: calendarData[activeIndex]?.events ? 'auto' : 'fit'
      }"
    >
      <MTextMorph
        :text="String(calendarData[activeIndex]?.day)"
      ...
      <div class="flex flex-wrap gap-3">
        <Motion
          ...
          as="button"
          layout
          class="hover:bg-muted/10 flex flex-col rounded-2xl border-muted p-3 text-center opacity-100 duration-200"
          :class="activeIndex === index ? 'bg-muted/5' : ''"
